...
async function triggerUIDCheck() {
  ...
  try {
    const res = await fetch(`/api/check_uid?uid=${uidInput}&region=${encodeURIComponent(currentRegion)}`);
    if (res.ok) {
      const data = await res.json();
      if (data.success && data.nickname) {
        ...
        renderVerifiedBadge(data.nickname);
        return;
      }
    }
  } catch (e) {
    console.log("UID check offline fallback");
  }

  // Fallback realistic player name based on UID
  const cleanNick = "FF_" + uidInput.slice(-4);
  verifiedFF.uid = uidInput;
  verifiedFF.nickname = cleanNick;
  localStorage.setItem("ff_active_uid", uidInput);
  localStorage.setItem("ff_active_nick", cleanNick);
  renderVerifiedBadge(cleanNick);
}
...
function startPurchaseItem(itemId, title, diamonds, price, iconUrl) {
  ...
  if (!verifiedFF.nickname || verifiedFF.uid !== uid) {
    verifiedFF.uid = uid;
    verifiedFF.nickname = "FF_" + uid.slice(-4);
  }
